...
比如说 `<p>` 标签就是段落的意思，在 HTML5 中所有的标签都有它书面上的语义。而没有语义的标签在 HTML 中有，在 HTML5 中就没有了，比如 `<font>` 标签。而 HTML5 又为了补充 HTML 现在的不足而加上了一些新的标签，如 `<article>`、`<footer>`、`<header>` 等等。而这些标签的分类叫法也从“块级元素”与“行内元素”变成了“外围内容”与“短语内容”。

在 HTML5 中最基本的理念就是“语义”与“显示”分离，HTML5 代码中出现的标签都是让开发人员或浏览器一看就知道是干什么用的东西，至于显示效果那是 CSS3（Cascading Style Sheet）需要干的事情。
